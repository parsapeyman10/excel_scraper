...
### ساخت نسخهٔ اجرایی ویندوز (اپ) — با آیکون
روی سیستم خودتان (ویندوز):
1. فایل آیکون — `web-data-scraping-icon-svg-download-png-3587064.png` (یا `.ico`) —
   را در پوشهٔ پروژه (کنار `app.spec`) بگذارید. نام `app_icon.*` هم پشتیبانی می‌شود.
2. روی **`build_app.bat`** دابل‌کلیک کنید (یا در ترمینال: `build_app.bat`).
3. خروجی: **`dist\BOM Validator.exe`** — تک‌فایل، بدون کنسول، با آیکون روی خودِ فایل
   exe و پنجره/تسک‌بار برنامه.

نکته‌ها:
* فقط همین **یک فایل exe** را به مشتری بدهید؛ `license_generator.py` عمداً داخل
  exe قرار نمی‌گیرد و **هرگز** نباید به مشتری برسد.
* اگر آنتی‌ویروس روی exe تک‌فایلی هشدار کاذب داد، طبیعی است (مشکل شناخته‌شدهٔ
  PyInstaller)؛ خروجی را در آنتی‌ویروس مشتری Allow کنید.
* بعد از هر تغییر در کد، دوباره `build_app.bat` را اجرا کنید.
